...
@section('title', 'All Courses - Paradise IT Solutions')
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($courses as $key => $blog)
                            <tr>
                                ...
                                <td>
                                    <a class="btn btn-sm btn-success" href="{{ route('coursesingle', $blog->slug) }}"
                                        target="_blank" style="float: left;margin-right: 5px;"><i
                                            class="fa-solid fa-eye"></i></a>
                                    <a class="btn btn-sm btn-primary" href="{{ route('course.edit', $blog->id) }}"
                                        style="float: left;margin-right: 5px;"><i class="fa-solid fa-pen-to-square"></i></a>

                                    <form class="delete-form" action="{{ route('course.destroy', $blog->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
